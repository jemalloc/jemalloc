...
arena_stats_merge(tsdn_t *tsdn, arena_t *arena, unsigned *nthreads,
	...
	lockedstat_inc_zu_unsynchronized(&astats->pa_shard_stats.mapped,
	    base_mapped + lockedstat_read_zu(tsdn,
	    LOCKEDSTAT_MTX(*arena->pa_shard.stats_mtx),
	    &arena->pa_shard.stats->mapped));
	lockedstat_inc_zu_unsynchronized(&astats->retained,
	...
	for (szind_t i = 0; i < SC_NSIZES - SC_NBINS; i++) {
		uint64_t nmalloc = lockedstat_read_u64(tsdn,
		    LOCKEDSTAT_MTX(arena->stats.mtx),
		    &arena->stats.lstats[i].nmalloc);
		lockedstat_inc_u64_unsynchronized(&lstats[i].nmalloc, nmalloc);
		lockedstat_inc_u64_unsynchronized(&astats->nmalloc_large,
		    nmalloc);

		uint64_t ndalloc = lockedstat_read_u64(tsdn,
		    LOCKEDSTAT_MTX(arena->stats.mtx),
		    &arena->stats.lstats[i].ndalloc);
		lockedstat_inc_u64_unsynchronized(&lstats[i].ndalloc, ndalloc);
		lockedstat_inc_u64_unsynchronized(&astats->ndalloc_large,
		    ndalloc);

		uint64_t nrequests = lockedstat_read_u64(tsdn,
		    LOCKEDSTAT_MTX(arena->stats.mtx),
		    &arena->stats.lstats[i].nrequests);
		lockedstat_inc_u64_unsynchronized(&lstats[i].nrequests,
		    nmalloc + nrequests);
		lockedstat_inc_u64_unsynchronized(&astats->nrequests_large,
		    nmalloc + nrequests);

		/* nfill == nmalloc for large currently. */
		lockedstat_inc_u64_unsynchronized(&lstats[i].nfills, nmalloc);
		lockedstat_inc_u64_unsynchronized(&astats->nfills_large,
		    nmalloc);

		uint64_t nflush = lockedstat_read_u64(tsdn,
		    LOCKEDSTAT_MTX(arena->stats.mtx),
		    &arena->stats.lstats[i].nflushes);
		lockedstat_inc_u64_unsynchronized(&lstats[i].nflushes, nflush);
		lockedstat_inc_u64_unsynchronized(&astats->nflushes_large,
		    nflush);
...
arena_new(tsdn_t *tsdn, unsigned ind, extent_hooks_t *extent_hooks) {
	...
	if (pa_shard_init(tsdn, &arena->pa_shard, base, ind,
	    &arena->stats.pa_shard_stats, LOCKEDSTAT_MTX(arena->stats.mtx))) {
		goto label_error;
	}
