...
extent_t *
base_alloc_extent(tsdn_t *tsdn, base_t *base, extent_class_t extent_class) {
	size_t esn;

	size_t sz = sizeof(extent_t);
	/* If this extent_t needs a bitmap, we add space for it. */
	if (extent_class == extent_class_small) {
		sz += SLAB_SMALL_BYTES;
	} else if (extent_class == extent_class_large) {
		size_t bitmap_sz = (bin_max_regs % 8 == 0) ?
		    bin_max_regs / 8 : bin_max_regs / 8 + 1;
		sz += bitmap_sz;
	}
