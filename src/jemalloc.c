...
zero_realloc_action_t opt_zero_realloc_action = zero_realloc_action_strict;
    ...
    if (CONF_MATCH(n)) {                                                       \
        ...
        CONF_CONTINUE;                                                         \
            ...
            if (CONF_MATCH("narenas_ratio")) {
                CONF_HANDLE_UNSIGNED(opt_narenas_ratio, "narenas_ratio", 1,
            ...
            if (CONF_MATCH("zero_realloc")) {
                if (CONF_MATCH_VALUE("strict")) {
                    opt_zero_realloc_action = zero_realloc_action_strict;
                } else if (CONF_MATCH_VALUE("free")) {
...
malloc_init_hard_a0_locked() {
     ...
     * arena_choose_hard().
     */
    if (arena_init(TSDN_NULL, 0, (extent_hooks_t *)&ehooks_default_extent_hooks)
      == NULL) {
        return true;
    }
...
batch_alloc(void **ptrs, size_t num, size_t size, int flags) {
    ...
    while (filled < num) {
        ...
        size_t n = arena_fill_small_fresh(
          tsd_tsdn(tsd), arena, ind, ptrs + filled, batch, zero);
        filled += n;
