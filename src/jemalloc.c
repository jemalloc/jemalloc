...
arena_choose_hard(tsd_t *tsd, bool internal) {
  ...
  if (narenas_auto > 1) {
    ...
    for (j = 0; j < 2; j++) {
      if (arena_nthreads_get(arena_get(tsd_tsdn(tsd), choose[j], false), !!j) ==
              0 ||
          first_null == narenas_auto) {
        /*
  ...
  if (CONF_MATCH(n)) {                                                         \
    ...
    continue;                                                                  \
      ...
      CONF_HANDLE_SIZE_T(opt_lg_extent_max_active_fit,
          "lg_extent_max_active_fit", 0, (sizeof(size_t) << 3), yes, yes, false)
      CONF_HANDLE_SSIZE_T(
      ...
      CONF_HANDLE_SIZE_T(opt_max_background_threads, "max_background_threads",
          1, opt_max_background_threads, yes, yes, true);
      if (config_prof) {
...
malloc_init_narenas(void) {
  ...
  if (opt_percpu_arena != percpu_arena_disabled) {
    if (!have_percpu_arena || malloc_getcpu() < 0) {
      ...
      unsigned n =
          percpu_arena_ind_limit(percpu_arena_as_initialized(opt_percpu_arena));
      if (opt_narenas < n) {
        /*
...
imalloc_sample(static_opts_t *sopts, dynamic_opts_t *dopts, tsd_t *tsd,
  ...
  if (usize <= SMALL_MAXCLASS) {
    assert(((dopts->alignment == 0) ? sz_s2u(LARGE_MINCLASS)
                                    : sz_sa2u(LARGE_MINCLASS,
                                          dopts->alignment)) == LARGE_MINCLASS);
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR
    JEMALLOC_RESTRICT_RETURN void JEMALLOC_NOTHROW *
...
irealloc_prof_sample(tsd_t *tsd, void *old_ptr, size_t old_usize, size_t usize,
  ...
  if (usize <= SMALL_MAXCLASS) {
    p = iralloc(tsd, old_ptr, old_usize, LARGE_MINCLASS, 0, false, hook_args);
    if (p == NULL) {
      return NULL;
    }
...
irallocx_prof_sample(tsdn_t *tsdn, void *old_ptr, size_t old_usize,
  ...
  if (usize <= SMALL_MAXCLASS) {
    p = iralloct(tsdn, old_ptr, old_usize, LARGE_MINCLASS, alignment, zero,
...
JEMALLOC_EXPORT JEMALLOC_ALLOCATOR
  ...
  if (config_prof && opt_prof) {
    ...
    p = iralloct(tsd_tsdn(tsd), ptr, old_usize, size, alignment, zero, tcache,
        arena, &hook_args);
    if (unlikely(p == NULL)) {
      goto label_oom;
    }
...
JEMALLOC_EXPORT size_t JEMALLOC_NOTHROW
JEMALLOC_ATTR(pure) je_sallocx(const void *ptr, UNUSED int flags) {
  size_t usize;
  tsdn_t *tsdn;
