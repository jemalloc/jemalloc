...
extent_dss_extending_start(void) {
  ...
  while (true) {
    bool expected = false;
    if (atomic_compare_exchange_weak_b(
            &dss_extending, &expected, true, ATOMIC_ACQ_REL, ATOMIC_RELAXED)) {
      break;
    }
    ...
    while (true) {
      ...
      if (gap_size_page != 0) {
        extent_init(gap, arena, gap_addr_page, gap_size_page, false, NSIZES,
            arena_extent_sn_next(arena), extent_state_active, false, true,
            true);
      }
      /*
      ...
      if (dss_prev == max_cur) {
        ...
        if (*zero && *commit) {
          ...
          extent_init(&extent, arena, ret, size, size, false, NSIZES,
