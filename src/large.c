...
large_palloc(
  ...
  if (unlikely(arena == NULL) || (extent = arena_extent_alloc_large(tsdn, arena,
  ...
  if (zero) {
    ...
    memset(
        extent_addr_get(extent), JEMALLOC_ALLOC_JUNK, extent_usize_get(extent));
  }
...
large_ralloc_no_move_shrink(tsdn_t *tsdn, extent_t *extent, size_t usize) {
  ...
  /* Split excess pages. */
  if (diff != 0) {
    extent_t *trail = extent_split_wrapper(tsdn, arena, &extent_hooks, extent,
        usize + sz_large_pad, sz_size2index(usize), false, diff, NSIZES, false);
    if (trail == NULL) {
      return true;
    }
...
large_ralloc(tsdn_t *tsdn, arena_t *arena, void *ptr, size_t usize,
  ...
  if (!large_ralloc_no_move(tsdn, extent, usize, usize, zero)) {
    hook_invoke_expand(
        hook_args->is_realloc ? hook_expand_realloc : hook_expand_rallocx, ptr,
        oldusize, usize, (uintptr_t)ptr, hook_args->args);
    return extent_addr_get(extent);
  }
