...
prof_dump_file(tsd_t *tsd, bool propagate_err, const char *filename,
  ...
  /* Dump profile header. */
  if (prof_dump_header(
          tsd_tsdn(tsd), propagate_err, &prof_tdata_merge_iter_arg->cnt_all)) {
...
prof_dump_filename(char *filename, char v, uint64_t vseq) {
  ...
  if (vseq != VSEQ_INVALID) {
    ...
    malloc_snprintf(filename, DUMP_FILENAME_BUFSIZE,
        "%s.%d.%" FMTu64 ".%c%" FMTu64 ".heap", opt_prof_prefix, prof_getpid(),
...
prof_tdata_expire(tsdn_t *tsdn, prof_tdata_t *tdata) {
  ...
  if (!tdata->expired) {
    tdata->expired = true;
    destroy_tdata =
        tdata->attached ? false : prof_tdata_should_destroy(tsdn, tdata, false);
  } else {
    destroy_tdata = false;
  }
