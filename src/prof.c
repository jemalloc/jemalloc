...
void
prof_free_sampled_prod_cons(prof_tdata_t *prod_tdata, prof_tdata_t *cons_tdata,
    size_t usize) {
	tsd_t *prod_tsd = prod_tdata->tsd;
	tsd_t *cons_tsd = cons_tdata->tsd;
	...
	if (ckh_search(&prod_cons, &pair, NULL, (void **)(&data))) {
		...
		data = iallocztm(tsd_tsdn(cons_tsd),
		    sizeof(prof_prod_cons_data_t),
		    sz_size2index(sizeof(prof_prod_cons_data_t)), true, NULL,
		    true, arena_get(TSDN_NULL, 0, true), true);

		data->prod_uid = prod_tdata->thr_uid;
		data->cons_uid = cons_tdata->thr_uid;


		ckh_insert(cons_tsd, &prod_cons, key, data);
	}

	data->bytes += usize;
	data->objs++;

	malloc_mutex_unlock(tsd_tsdn(cons_tsd), &prod_cons_mtx);
}
...
prof_free_sampled_object(tsd_t *tsd, size_t usize, prof_tctx_t *tctx) {
	...
	/*
	 * This can happen if tsd is in an unusual state: e.g. the current
	 * thread will be in state tsd_state_purgatory if this thread is
	 * currently being destroyed. We just ignore these cases.
	 */
	if (cons_tdata != NULL) {
		prof_free_sampled_prod_cons(tctx->tdata, cons_tdata, usize);
	}
