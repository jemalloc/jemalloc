...
stats_arena_bins_print(emitter_t *emitter, bool mutex, unsigned i,
	...
	COL_HDR(row, pop_try, NULL, right, 10, uint64)
	COL_HDR(row, pop_try_ps, "(#/sec)", right, 8, uint64)
	COL_HDR(row, pop_success, NULL, right, 13, uint64)
	COL_HDR(row, pop_success_ps, "(#/sec)", right, 8, uint64)
	COL_HDR(row, push, NULL, right, 7, uint64)
	COL_HDR(row, push_ps, "(#/sec)", right, 8, uint64)
	COL_HDR(row, push_failed, NULL, right, 7, uint64)
	COL_HDR(row, push_failed_ps, "(#/sec)", right, 8, uint64)
	...
	for (j = 0, in_gap = false; j < nbins; j++) {
		...
		uint64_t nreslabs;
		uint64_t batch_pop_attempts, batch_pop_successes, batch_pushes,
			 batch_failed_push_attempts, batch_pushed_elems;
		...
		CTL_LEAF(stats_arenas_mib, 5, "batch_pop_attempts",
		    &batch_pop_attempts, uint64_t);
		CTL_LEAF(stats_arenas_mib, 5, "batch_pop_successes",
		    &batch_pop_successes, uint64_t);
		CTL_LEAF(stats_arenas_mib, 5, "batch_pushes",
		    &batch_pushes, uint64_t);
		CTL_LEAF(stats_arenas_mib, 5, "batch_failed_push_attempts",
		    &batch_failed_push_attempts, uint64_t);
		...
		emitter_json_kv(emitter, "nonfull_slabs", emitter_type_size,
		    &nonfull_slabs);
		emitter_json_kv(emitter, "batch_pop_attempts",
		    emitter_type_uint64, &batch_pop_attempts);
		emitter_json_kv(emitter, "batch_pop_successes",
		    emitter_type_uint64, &batch_pop_successes);
		emitter_json_kv(emitter, "batch_pushes",
		    emitter_type_uint64, &batch_pushes);
		emitter_json_kv(emitter, "batch_failed_push_attempts",
		    emitter_type_uint64, &batch_failed_push_attempts);
		...
		col_pop_try.uint64_val = batch_pop_attempts;
		col_pop_try_ps.uint64_val
		    = rate_per_second(batch_pop_attempts, uptime);
		col_pop_success.uint64_val = batch_pop_successes;
		col_pop_success_ps.uint64_val
		    = rate_per_second(batch_pop_successes, uptime);
		col_push.uint64_val = batch_pushes;
		col_push_ps.uint64_val
		    = rate_per_second(batch_pushes, uptime);

		col_push_failed.uint64_val = batch_failed_push_attempts;
		col_push_failed_ps.uint64_val
		    = rate_per_second(batch_failed_push_attempts, uptime);
...
#undef MALLOC_CONF_WRITE
	...
	OPT_WRITE_SIZE_T("batches_per_bin")
