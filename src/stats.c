...
#define OP(counter, counter_type, human)                                       \
  ...
  dst->type = EMITTER_TYPE_##counter_type;                                     \
  gen_mutex_ctl_str(                                                           \
      cmd, MUTEX_CTL_STR_MAX_LENGTH, "arenas.0.bins.0", "mutex", #counter);    \
  CTL_M2_M4_GET(                                                               \
      cmd, arena_ind, bin_ind, (counter_type *)&dst->bool_val, counter_type);
  MUTEX_PROF_COUNTERS
#undef OP
#undef EMITTER_TYPE_uint32_t
#undef EMITTER_TYPE_uint64_t
...
#define OP(counter, type, human)                                               \
  ...
  ++k_##type;                                                                  \
  emitter_json_kv(                                                             \
      emitter, #counter, EMITTER_TYPE_##type, (const void *)&col->bool_val);
  MUTEX_PROF_COUNTERS;
#undef OP
#undef EMITTER_TYPE_uint32_t
#undef EMITTER_TYPE_uint64_t
...
static void
stats_arena_print(
    emitter_t *emitter, unsigned i, bool bins, bool large, bool mutex) {
  unsigned nthreads;
  const char *dss;
...
#define OPT_WRITE_MUTABLE(name, var1, var2, size, emitter_type, altname)       \
  if (je_mallctl("opt." name, (void *)&var1, &size, NULL, 0) == 0 &&           \
    ...
    emitter_kv_note(emitter, name, "opt." name, emitter_type, &var1, altname,  \
        emitter_type, &var2);                                                  \
  }
...
stats_print_helper(emitter_t *emitter, bool merged, bool destroyed,
  ...
  if (have_background_thread) {
    CTL_GET(
        "stats.background_thread.num_threads", &num_background_threads, size_t);
...
#undef OPTION
  ...
  if (config_stats) {
    stats_print_helper(
        &emitter, merged, destroyed, unmerged, bins, large, mutex);
  }
