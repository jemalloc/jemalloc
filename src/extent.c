...
ph_gen(
    UNUSED, extent_avail_, extent_tree_t, extent_t, ph_link, extent_esnead_comp)
...
extent_rtree_leaf_elm_try_lock(
    tsdn_t *tsdn, rtree_leaf_elm_t *elm, extent_t **result) {
  extent_t *extent1 =
...
extents_remove_locked(tsdn_t *tsdn, extents_t *extents, extent_t *extent) {
  ...
  atomic_store_zu(
      &extents->npages, cur_extents_npages - (size >> LG_PAGE), ATOMIC_RELAXED);
}
...
extents_best_fit_locked(
  ...
  pszind_t i =
      (pszind_t)bitmap_ffu(extents->bitmap, &extents_bitmap_info, (size_t)pind);
  if (i < NPSIZES + 1) {
    /*
...
extents_first_fit_locked(
  ...
  for (pszind_t i = (pszind_t)bitmap_ffu(
           extents->bitmap, &extents_bitmap_info, (size_t)pind);
       i < NPSIZES + 1; i = (pszind_t)bitmap_ffu(extents->bitmap,
                            &extents_bitmap_info, (size_t)i + 1)) {
    assert(!extent_heap_empty(&extents->heaps[i]));
    extent_t *extent = extent_heap_first(&extents->heaps[i]);
    assert(extent_size_get(extent) >= size);
...
extent_try_delayed_coalesce(tsdn_t *tsdn, arena_t *arena,
  ...
  bool coalesced;
  extent = extent_try_coalesce(tsdn, arena, r_extent_hooks, rtree_ctx, extents,
...
extents_leak(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
  ...
  if (extents_state_get(extents) == extent_state_dirty) {
    if (extent_purge_lazy_impl(tsdn, arena, r_extent_hooks, extent, 0,
...
extent_interior_register(
  ...
  for (size_t i = 1; i < (extent_size_get(extent) >> LG_PAGE) - 1; i++) {
    rtree_write(tsdn, &extents_rtree, rtree_ctx,
        (uintptr_t)extent_base_get(extent) + (uintptr_t)(i << LG_PAGE), extent,
        szind, true);
  }
}
...
extent_gdump_add(tsdn_t *tsdn, const extent_t *extent) {
  ...
  if (opt_prof && extent_state_get(extent) == extent_state_active) {
    ...
    while (cur > high && !atomic_compare_exchange_weak_zu(&highpages, &high,
                             cur, ATOMIC_RELAXED, ATOMIC_RELAXED)) {
      /*
...
extent_split_interior(tsdn_t *tsdn, arena_t *arena,
  ...
  *to_salvage = NULL;
  ...
  if (leadsize != 0) {
    *lead = *extent;
    *extent = extent_split_impl(tsdn, arena, r_extent_hooks, *lead, leadsize,
        NSIZES, false, esize + trailsize, szind, slab, growing_retained);
    if (*extent == NULL) {
      *to_leak = *lead;
      *lead = NULL;
  ...
  if (trailsize != 0) {
    *trail = extent_split_impl(tsdn, arena, r_extent_hooks, *extent, esize,
        szind, slab, trailsize, NSIZES, false, growing_retained);
    if (*trail == NULL) {
      *to_leak = *extent;
      *to_salvage = *lead;
      *lead = NULL;
...
extent_recycle_split(tsdn_t *tsdn, arena_t *arena,
  ...
  extent_split_interior_result_t result = extent_split_interior(tsdn, arena,
      r_extent_hooks, rtree_ctx, &extent, &lead, &trail, &to_leak, &to_salvage,
...
extent_recycle(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
  ...
  extent = extent_recycle_split(tsdn, arena, r_extent_hooks, rtree_ctx, extents,
      new_addr, size, pad, alignment, slab, szind, extent, growing_retained);
  if (extent == NULL) {
    return NULL;
  }
...
extent_alloc_core(tsdn_t *tsdn, arena_t *arena, void *new_addr, size_t size,
  ...
  if (have_dss && dss_prec == dss_prec_primary &&
      (ret = extent_alloc_dss(
           tsdn, arena, new_addr, size, alignment, zero, commit)) != NULL) {
    return ret;
  }
  /* mmap. */
...
extent_alloc_default(extent_hooks_t *extent_hooks, void *new_addr, size_t size,
  ...
  return extent_alloc_default_impl(
...
extent_grow_retained(tsdn_t *tsdn, arena_t *arena,
  ...
  if (extent_register_no_gdump_add(tsdn, extent)) {
    extents_leak(
        tsdn, arena, r_extent_hooks, &arena->extents_retained, extent, true);
    goto label_err;
  }
  ...
  if (result == extent_split_interior_ok) {
    ...
    if (to_leak != NULL) {
      extent_deregister_no_gdump_sub(tsdn, to_leak);
      extents_leak(
          tsdn, arena, r_extent_hooks, &arena->extents_retained, to_leak, true);
    }
    goto label_err;
  }
...
extent_alloc_wrapper_hard(tsdn_t *tsdn, arena_t *arena,
  ...
  if (*r_extent_hooks == &extent_hooks_default) {
    ...
    addr = extent_alloc_default_impl(
        tsdn, arena, new_addr, esize, alignment, zero, commit);
  } else {
    extent_hook_pre_reentrancy(tsdn, arena);
    addr = (*r_extent_hooks)
               ->alloc(*r_extent_hooks, new_addr, esize, alignment, zero,
...
extent_alloc_wrapper(tsdn_t *tsdn, arena_t *arena,
  ...
  if (extent == NULL) {
    ...
    extent = extent_alloc_wrapper_hard(tsdn, arena, r_extent_hooks, new_addr,
...
extent_try_coalesce(tsdn_t *tsdn, arena_t *arena,
  ...
  do {
    ...
    extent_t *next =
        extent_lock_from_addr(tsdn, rtree_ctx, extent_past_get(extent));
    if (next != NULL) {
      /*
      ...
      if (can_coalesce && !extent_coalesce(tsdn, arena, r_extent_hooks, extents,
    ...
    extent_t *prev =
        extent_lock_from_addr(tsdn, rtree_ctx, extent_before_get(extent));
    if (prev != NULL) {
...
extent_record(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
  ...
  if (!extents->delay_coalesce) {
    ...
    do {
      ...
      assert(extent_state_get(extent) == extent_state_active);
      extent = extent_try_coalesce(tsdn, arena, r_extent_hooks, rtree_ctx,
          extents, extent, &coalesced, growing_retained);
    } while (
        coalesced && extent_size_get(extent) >= prev_size + LARGE_MINCLASS);
  }
  extent_deactivate_locked(tsdn, arena, extents, extent);
...
extent_dalloc_gap(tsdn_t *tsdn, arena_t *arena, extent_t *extent) {
  ...
  if (extent_register(tsdn, extent)) {
    extents_leak(
        tsdn, arena, &extent_hooks, &arena->extents_retained, extent, false);
    return;
  }
 ...
 */
static extent_t *
extent_split_impl(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
...
extent_split_wrapper(tsdn_t *tsdn, arena_t *arena,
    extent_hooks_t **r_extent_hooks, extent_t *extent, size_t size_a,
    szind_t szind_a, bool slab_a, size_t size_b, szind_t szind_b, bool slab_b) {
  return extent_split_impl(tsdn, arena, r_extent_hooks, extent, size_a, szind_a,
...
static bool
extent_merge_impl(tsdn_t *tsdn, arena_t *arena, extent_hooks_t **r_extent_hooks,
...
extent_boot(void) {
  ...
  if (mutex_pool_init(
          &extent_mutex_pool, "extent_mutex_pool", WITNESS_RANK_EXTENT_POOL)) {
    return true;
  }
