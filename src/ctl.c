...
#define CTL_PROTO(n)                                                           \
  static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, \
...
static const ctl_named_node_t prof_node[] = {
    ...
    {NAME("gdump"), CTL(prof_gdump)}, {NAME("reset"), CTL(prof_reset)},
...
ctl_arena_clear(ctl_arena_t *ctl_arena) {
  ...
  if (config_stats) {
    ...
    memset(ctl_arena->astats->lstats, 0,
        (NSIZES - NBINS) * sizeof(arena_stats_large_t));
  }
}
...
#undef OP
    ...
    if (!destroyed) {
      ...
      assert(
          atomic_load_zu(&astats->astats.allocated_large, ATOMIC_RELAXED) == 0);
    }
...
ctl_refresh(tsdn_t *tsdn) {
  ...
  if (config_stats) {
    ...
    ctl_stats->resident =
        atomic_load_zu(&ctl_sarena->astats->astats.resident, ATOMIC_RELAXED);
    ctl_stats->mapped =
        atomic_load_zu(&ctl_sarena->astats->astats.mapped, ATOMIC_RELAXED);
    ctl_stats->retained =
        atomic_load_zu(&ctl_sarena->astats->astats.retained, ATOMIC_RELAXED);
...
#define READ_GLOBAL_MUTEX_PROF_DATA(i, mtx)                                    \
  ...
  malloc_mutex_unlock(tsdn, &mtx);
    ...
    if (have_background_thread) {
      ...
      memset(&ctl_stats->mutex_prof_data[global_prof_mutex_background_thread],
...
ctl_init(tsd_t *tsd) {
  ...
  if (!ctl_initialized) {
    ...
    if ((ctl_darena = arenas_i_impl(
             tsd, MALLCTL_ARENAS_DESTROYED, false, true)) == NULL) {
      ret = true;
      goto label_return;
    }
...
arena_i_dss_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp,
  ...
  if (arena_ind == MALLCTL_ARENAS_ALL || arena_ind == ctl_arenas->narenas) {
    ...
    if (arena == NULL ||
        (dss_prec != dss_prec_limit && arena_dss_prec_set(arena, dss_prec))) {
      ret = EFAULT;
      goto label_return;
    }
...
CTL_RO_NL_GEN(
    arenas_lextent_i_size, sz_index2size(NBINS + (szind_t)mib[2]), size_t)
static const ctl_named_node_t *
...
CTL_RO_CGEN(config_stats, stats_arenas_i_large_ndalloc,
    ...
    uint64_t)
/*
 * Note: "nmalloc" here instead of "nrequests" in the read.  This is
 * intentional.
 */
...
#define RO_MUTEX_CTL_GEN(n, l)                                                 \
  ...
  CTL_RO_CGEN(config_stats, stats_##n##_num_owner_switch, l.n_owner_switches,  \
      uint64_t)                                                                \
