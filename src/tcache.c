...
tcache_bin_flush_impl_small(tsd_t *tsd, tcache_t *tcache, cache_bin_t *cache_bin,
	...
	while (flush_start < nflush) {
		 ...
		 */
		bool bin_is_batched = binind < bin_info_nbatched_sizes;
		bool home_binshard = (cur_arena == tcache_arena
		    && cur_binshard == tcache_binshard);
		bool can_batch = (flush_start - prev_flush_start < BIN_ELEMS_PER_BATCH)
		...
		if (can_batch && !locked) {
			...
			int push_idx = batcher_push_begin(
			    &batched_bin->remote_frees,
			    batched_bin->remote_free_elems);
			bin_batching_test_after_push(push_idx);
			if (push_idx != BATCHER_NO_IDX) {
				batched = true;
				bin_batch_data_t *batch_data =
				    &batched_bin->remote_free_data[push_idx];
				unsigned nbatched
				    = flush_start - prev_flush_start;
				for (unsigned i = 0; i < nbatched; i++) {
					unsigned src_ind = prev_flush_start + i;
					batch_data->elems[i].ptr
					    = ptrs->ptr[src_ind];
					batch_data->elems[i].slab
					    = item_edata[src_ind].edata;
				}
				for (unsigned i = nbatched;
				    i < BIN_ELEMS_PER_BATCH; i++) {
					batch_data->elems[i].ptr = NULL;
					batch_data->elems[i].slab = NULL;
				}
				batcher_push_end(&batched_bin->remote_frees,
				    batched_bin->remote_free_elems, push_idx);
				/*
				 * Note that the bin stats updates for number of
				 * successful pushes actually happen on the
				 * *pop* side.  It of course *has* to be this
				 * way: those stats are guarded by the bin
				 * mutex, and the whole point of batcher pushes
				 * is to avoid acquiring it.  Counts for failed
				 * pushes are accounted for below (after we
				 * acquire the lock).
				 */
			} else {
				batch_failed = true;
		...
		if (!batched) {
			...
			if (config_stats && batch_failed) {
				cur_bin->stats.batch_failed_push_attempts++;
			}
			...
			unsigned ndalloc_slabs = nflush;
			if (ndalloc_slabs
			    > bin_batching_test_ndalloc_slabs_max) {
				ndalloc_slabs
				    = bin_batching_test_ndalloc_slabs_max;
			}
