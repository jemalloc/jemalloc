...
#define KSMAX(x) ((uintmax_t)(intmax_t)x##LL)
  struct test_s tests[] = {{"0", "0", -1, ERR(EINVAL), UINTMAX_MAX},
      ...
      {"-42", "", 0, ERR(0), KSMAX(-42)}, {"042", "", 0, ERR(0), KUMAX(042)},
      ...
      {"42", "", 0, ERR(0), KUMAX(42)}, {" 42", "", 0, ERR(0), KUMAX(42)},
      {"42 ", " ", 0, ERR(0), KUMAX(42)}, {"0x", "x", 0, ERR(0), KUMAX(0)},
      ...
      {"0xg", "xg", 0, ERR(0), KUMAX(0)}, {"0XA", "", 0, ERR(0), KUMAX(10)},

      {"010", "", 10, ERR(0), KUMAX(10)}, {"0x3", "x3", 10, ERR(0), KUMAX(0)},

      {"12", "2", 2, ERR(0), KUMAX(1)}, {"78", "8", 8, ERR(0), KUMAX(7)},
      {"9a", "a", 10, ERR(0), KUMAX(9)}, {"9A", "A", 10, ERR(0), KUMAX(9)},
      {"fg", "g", 16, ERR(0), KUMAX(15)}, {"FG", "G", 16, ERR(0), KUMAX(15)},
      {"0xfg", "g", 16, ERR(0), KUMAX(15)},
      {"0XFG", "G", 16, ERR(0), KUMAX(15)}, {"z_", "_", 36, ERR(0), KUMAX(35)},
...
#undef KSMAX
  ...
  for (i = 0; i < sizeof(tests) / sizeof(struct test_s); i++) {
    ...
    assert_d_eq(err, test->expected_errno,
        "Expected errno %s for \"%s\", base %d", test->expected_errno_name,
        test->input, test->base);
    assert_str_eq(remainder, test->expected_remainder,
        "Unexpected remainder for \"%s\", base %d", test->input, test->base);
    if (err == 0) {
      assert_ju_eq(result, test->expected_x,
          "Unexpected result for \"%s\", base %d", test->input, test->base);
    }
  }
}
TEST_END
...
#define TEST(expected_str_untruncated, ...)                                    \
  do {                                                                         \
    ...
    assert_zu_eq(                                                              \
        result, strlen(expected_str_untruncated), "Unexpected result");        \
  } while (0)
