...
TEST_BEGIN(test_reserve_alloc) {
    ...
    for (size_t i = 0; i < HUGEPAGE_PAGES; i++) {
        ...
        expect_zu_eq(
          HUGEPAGE_PAGES - i, hpdata_longest_free_range_get(&hpdata), "");
...
TEST_BEGIN(test_purge_intervening_dalloc) {
    ...
    got_result =
      hpdata_purge_next(&hpdata, &purge_state, &purge_addr, &purge_size);
    expect_true(got_result, "");
    expect_ptr_eq((void *)((uintptr_t)alloc + 2 * HUGEPAGE_PAGES / 4 * PAGE),
      purge_addr, "");
...
main(void) {
    return test_no_reentrancy(test_reserve_alloc, test_purge_simple,
      test_purge_intervening_dalloc, test_purge_over_retained, test_hugify);
}
