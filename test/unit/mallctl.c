...
#define NTCACHES 10
  ...
  for (i = 0; i < NTCACHES; i++) {
    assert_d_eq(
        mallctl("tcache.flush", NULL, NULL, (void *)&tis[i], sizeof(unsigned)),
        0, "Unexpected mallctl() failure, i=%u", i);
  }
  ...
  for (i = 0; i < NTCACHES; i++) {
    ...
    assert_ptr_not_null(ps[i], "Unexpected mallocx() failure, i=%u", i);
    assert_ptr_eq(
        ps[i], p0, "Expected mallocx() to allocate cached region, i=%u", i);
  }
  ...
  for (i = 0; i < NTCACHES; i++) {
    ...
    assert_ptr_eq(
        qs[i], q0, "Expected rallocx() to allocate cached region, i=%u", i);
...
TEST_BEGIN(test_thread_arena) {
  ...
  if (strcmp(opa, "disabled") == 0) {
    ...
    assert_d_eq(mallctl("thread.arena", (void *)&old_arena_ind, &sz, NULL, 0),
        0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_dirty_decay_ms) {
  ...
  for (prev_dirty_decay_ms = dirty_decay_ms, dirty_decay_ms = -1;
    ...
    assert_d_eq(mallctl("arena.0.dirty_decay_ms", (void *)&old_dirty_decay_ms,
                    &sz, (void *)&dirty_decay_ms, sizeof(ssize_t)),
        0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_muzzy_decay_ms) {
  ...
  for (prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms = -1;
    ...
    assert_d_eq(mallctl("arena.0.muzzy_decay_ms", (void *)&old_muzzy_decay_ms,
                    &sz, (void *)&muzzy_decay_ms, sizeof(ssize_t)),
        0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arenas_muzzy_decay_ms) {
  ...
  for (prev_muzzy_decay_ms = muzzy_decay_ms, muzzy_decay_ms = -1;
    ...
    assert_d_eq(mallctl("arenas.muzzy_decay_ms", (void *)&old_muzzy_decay_ms,
                    &sz, (void *)&muzzy_decay_ms, sizeof(ssize_t)),
        0, "Unexpected mallctl() failure");
    assert_zd_eq(old_muzzy_decay_ms, prev_muzzy_decay_ms,
...
#define TEST_ARENAS_BIN_CONSTANT(t, name, expected)                            \
  do {                                                                         \
    t name;                                                                    \
    size_t sz = sizeof(t);                                                     \
    assert_d_eq(mallctl("arenas.bin.0." #name, (void *)&name, &sz, NULL, 0),   \
        0, "Unexpected mallctl() failure");                                    \
    assert_zu_eq(name, expected, "Incorrect " #name " size");                  \
  } while (0)
...
TEST_BEGIN(test_arenas_create) {
  ...
  assert_d_eq(mallctl("arenas.narenas", (void *)&narenas_before, &sz, NULL, 0),
      0, "Unexpected mallctl() failure");
...
#define TEST_STATS_ARENAS(t, name)                                             \
  do {                                                                         \
    t name;                                                                    \
    size_t sz = sizeof(t);                                                     \
    assert_d_eq(mallctl("stats.arenas.0." #name, (void *)&name, &sz, NULL, 0), \
        0, "Unexpected mallctl() failure");                                    \
  } while (0)
...
main(void) {
  return test(test_mallctl_errors, test_mallctlnametomib_errors,
      ...
      test_mallctlnametomib_short_mib, test_mallctl_config, test_mallctl_opt,
      test_manpage_example, test_tcache_none, test_tcache, test_thread_arena,
      ...
      test_stats_arenas, test_hooks, test_hooks_exhaustion);
}
