...
TEST_BEGIN(test_rtree_extrema) {
    ...
    edata_init(edata_a, INVALID_ARENA_IND, NULL, SC_LARGE_MINCLASS, false,
      sz_size2index(SC_LARGE_MINCLASS), 0, extent_state_active, false, false,
    ...
    expect_false(rtree_write(tsdn, rtree, &rtree_ctx, PAGE, contents_a),
      "Unexpected rtree_write() failure");
    rtree_contents_t read_contents_a =
      rtree_read(tsdn, rtree, &rtree_ctx, PAGE);
    ...
    expect_false(
      ...
      "Unexpected rtree_write() failure");
    rtree_contents_t read_contents_b =
      rtree_read(tsdn, rtree, &rtree_ctx, ~((uintptr_t)0));
...
TEST_BEGIN(test_rtree_bits) {
    ...
    for (unsigned i = 0; i < sizeof(keys) / sizeof(uintptr_t); i++) {
        ...
        expect_false(rtree_write(tsdn, rtree, &rtree_ctx, keys[i], contents),
          "Unexpected rtree_write() failure");
        for (unsigned j = 0; j < sizeof(keys) / sizeof(uintptr_t); j++) {
            expect_ptr_eq(rtree_read(tsdn, rtree, &rtree_ctx, keys[j]).edata,
              edata_c,
        ...
        expect_ptr_null(
          rtree_read(tsdn, rtree, &rtree_ctx, (((uintptr_t)2) << LG_PAGE))
            .edata,
...
#define SEED 42
    ...
    for (unsigned i = 0; i < NSET; i++) {
        ...
        rtree_leaf_elm_t *elm =
          rtree_leaf_elm_lookup(tsdn, rtree, &rtree_ctx, keys[i], false, true);
        ...
        expect_ptr_eq(rtree_read(tsdn, rtree, &rtree_ctx, keys[i]).edata,
          edata_d, "rtree_edata_read() should return previously set value");
    }
    for (unsigned i = 0; i < NSET; i++) {
        expect_ptr_eq(rtree_read(tsdn, rtree, &rtree_ctx, keys[i]).edata,
          edata_d,
    ...
    for (unsigned i = 0; i < NSET; i++) {
        rtree_clear(tsdn, rtree, &rtree_ctx, keys[i]);
        expect_ptr_null(rtree_read(tsdn, rtree, &rtree_ctx, keys[i]).edata,
          "rtree_edata_read() should return previously set value");
    }
    for (unsigned i = 0; i < NSET; i++) {
        expect_ptr_null(rtree_read(tsdn, rtree, &rtree_ctx, keys[i]).edata,
...
test_rtree_range_write(
  tsdn_t *tsdn, rtree_t *rtree, uintptr_t start, uintptr_t end) {
    ...
    for (uintptr_t i = 0; i < ((end - start) >> LG_PAGE); i++) {
        expect_ptr_eq(
          rtree_read(tsdn, rtree, &rtree_ctx, start + (i << LG_PAGE)).edata,
          edata_e, "rtree_edata_read() should return previously set value");
    }
    rtree_clear_range(tsdn, rtree, &rtree_ctx, start, end);
    rtree_leaf_elm_t *elm;
    for (uintptr_t i = 0; i < ((end - start) >> LG_PAGE); i++) {
        elm = rtree_leaf_elm_lookup(
          tsdn, rtree, &rtree_ctx, start + (i << LG_PAGE), false, false);
        expect_ptr_not_null(elm, "Should have been initialized.");
        expect_ptr_null(rtree_leaf_elm_read(tsdn, rtree, elm, false).edata,
