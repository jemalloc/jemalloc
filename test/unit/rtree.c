...
TEST_BEGIN(test_rtree_extrema) {
  ...
  extent_init(&extent_a, NULL, NULL, LARGE_MINCLASS, false,
      sz_size2index(LARGE_MINCLASS), 0, extent_state_active, false, false,
      true);
  extent_init(&extent_b, NULL, NULL, 0, false, NSIZES, 0, extent_state_active,
      false, false, true);
  ...
  assert_ptr_eq(rtree_extent_read(tsdn, rtree, &rtree_ctx, PAGE, true),
      &extent_a, "rtree_extent_read() should return previously set value");
...
TEST_BEGIN(test_rtree_bits) {
  ...
  extent_t extent;
  extent_init(&extent, NULL, NULL, 0, false, NSIZES, 0, extent_state_active,
      false, false, true);
  ...
  for (unsigned i = 0; i < sizeof(keys) / sizeof(uintptr_t); i++) {
    assert_false(
        rtree_write(tsdn, rtree, &rtree_ctx, keys[i], &extent, NSIZES, false),
        "Unexpected rtree_write() failure");
    for (unsigned j = 0; j < sizeof(keys) / sizeof(uintptr_t); j++) {
      assert_ptr_eq(rtree_extent_read(tsdn, rtree, &rtree_ctx, keys[j], true),
          &extent,
...
#define SEED 42
  ...
  for (unsigned i = 0; i < NSET; i++) {
    ...
    assert_ptr_eq(rtree_extent_read(tsdn, rtree, &rtree_ctx, keys[i], true),
        &extent, "rtree_extent_read() should return previously set value");
  }
  for (unsigned i = 0; i < NSET; i++) {
    assert_ptr_eq(rtree_extent_read(tsdn, rtree, &rtree_ctx, keys[i], true),
        &extent,
