...
check_bins_info(cache_bin_info_t tcache_bin_info[TCACHE_NBINS_MAX]) {
    ...
    for (szind_t i = 0; i < TCACHE_NBINS_MAX; i++) {
        ...
        expect_d_eq(mallctlbymib(mib_get, mib_get_len, (void *)&ncached_max,
        ...
        expect_d_eq(
          mallctlbymib(mib_get, mib_get_len, (void *)&temp_ncached_max, &sz,
...
ncached_max_check(void *args) {
    ...
    memcpy(tcache_bin_info_backup, tcache_bin_info, sizeof(tcache_bin_info));
    /* Check ncached_max set by malloc_conf. */
    for (szind_t i = 0; i < TCACHE_NBINS_MAX; i++) {
        bool first_range =
          (i >= sz_size2index(256) && i <= sz_size2index(1024));
        ...
        if (first_range || second_range || third_range) {
            target_ncached_max = first_range ? 1001 : (second_range ? 0 : 1);
            expect_true(tcache_get_default_ncached_max_set(i),
              "Unexpected state for bin %u", i);
            expect_zu_eq(target_ncached_max, tcache_bin_info[i].ncached_max,
    ...
    char *inputp = inputs;
    expect_d_eq(mallctlbymib(mib_set, mib_set_len, NULL, NULL, (void *)&inputp,
                  sizeof(char *)),
      0, "Unexpected mallctlbymib() failure");
    for (szind_t i = 0; i < TCACHE_NBINS_MAX; i++) {
    ...
    memcpy(
      tcache_bin_info, tcache_bin_info_backup, sizeof(tcache_bin_info_backup));
    for (szind_t i = tcache_nbins_get(tcache_slow); i < TCACHE_NBINS_MAX; i++) {
        cache_bin_info_init(&tcache_bin_info[i], 0);
    }
    check_bins_info(tcache_bin_info);
    ...
    assert_d_eq(mallctl("thread.tcache.max", NULL, NULL, (void *)&tcache_max,
                  sizeof(size_t)),
