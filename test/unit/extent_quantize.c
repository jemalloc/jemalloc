...
TEST_BEGIN(test_small_extent_size) {
    ...
    for (i = 0; i < nbins; i++) {
        ...
        sz = sizeof(size_t);
        expect_d_eq(
          mallctlbymib(mib, miblen, (void *)&extent_size, &sz, NULL, 0), 0,
...
TEST_BEGIN(test_large_extent_size) {
    ...
    sz = sizeof(bool);
    expect_d_eq(
      mallctl("opt.cache_oblivious", (void *)&cache_oblivious, &sz, NULL, 0), 0,
      "Unexpected mallctl failure");
    ...
    for (i = 0; i < nlextents; i++) {
        ...
        sz = sizeof(size_t);
        expect_d_eq(
          mallctlbymib(mib, miblen, (void *)&lextent_size, &sz, NULL, 0), 0,
          "Unexpected mallctlbymib failure");
