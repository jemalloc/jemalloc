...
TEST_BEGIN(test_small_extent_size) {
  ...
  for (i = 0; i < nbins; i++) {
    mib[2] = i;
    sz = sizeof(size_t);
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&extent_size, &sz, NULL, 0),
...
TEST_BEGIN(test_large_extent_size) {
  ...
  sz = sizeof(bool);
  assert_d_eq(
      mallctl("config.cache_oblivious", (void *)&cache_oblivious, &sz, NULL, 0),
  ...
  for (i = 0; i < nlextents; i++) {
    ...
    mib[2] = i;
    sz = sizeof(size_t);
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&lextent_size, &sz, NULL, 0),
        0, "Unexpected mallctlbymib failure");
