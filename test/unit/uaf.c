...
uaf_detection_enabled(void) {
    ...
    assert_d_eq(mallctl("thread.tcache.enabled", &tcache_enabled, &sz, NULL, 0),
...
test_use_after_free(size_t alloc_size, bool write_after_free) {
    ...
    while (iter-- != 0) {
        ...
        if (*mem != magic) {
            junked = true;
            assert_c_eq(
              *mem, (char)uaf_detect_junk, "Unexpected junk-filling bytes");
            if (write_after_free) {
                *(char *)mem = magic + 1;
            }
...
TEST_BEGIN(test_use_after_free_integration) {
    ...
    *(void **)allocated[0] = allocated[n_alloc - 1];
    expect_true(
      check_allocated_intact(allocated, n_alloc), "Allocated data corrupted");
    ...
    /* Read-after-free */
    expect_false(
      check_allocated_intact(allocated, n_alloc), "Junk-filling not detected");
