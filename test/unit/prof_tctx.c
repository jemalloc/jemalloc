...
TEST_BEGIN(test_prof_realloc) {
  ...
  assert_u64_eq(
      curobjs_0 + 1, curobjs_1, "Allocation should have increased sample size");
  ...
  assert_u64_eq(
      curobjs_1, curobjs_2, "Reallocation should not have changed sample size");

  dallocx(q, flags);
  prof_cnt_all(&curobjs_3, NULL, NULL, NULL);
  assert_u64_eq(
      curobjs_0, curobjs_3, "Sample size should have returned to base level");
}
TEST_END
