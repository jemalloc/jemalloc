...
#undef NB
  ...
  static size_t test_bitmap_size_body(
...
test_bitmap_xfu_body(const bitmap_info_t *binfo, size_t nbits) {
    ...
    if (nbits >= 3) {
        for (size_t i = 0; i < nbits - 2; i++) {
            ...
            expect_zu_eq(
              bitmap_ffu(bitmap, binfo, i), i, "Unexpected first unset bit");
            expect_zu_eq(bitmap_ffu(bitmap, binfo, i + 1), i + 2,
              "Unexpected first unset bit");
            expect_zu_eq(bitmap_ffu(bitmap, binfo, i + 2), i + 2,
              "Unexpected first unset bit");
            if (i + 3 < nbits) {
                expect_zu_eq(bitmap_ffu(bitmap, binfo, i + 3), nbits,
                  "Unexpected first unset bit");
            }
            expect_zu_eq(
              bitmap_sfu(bitmap, binfo), i, "Unexpected first unset bit");
            expect_zu_eq(
              bitmap_sfu(bitmap, binfo), i + 2, "Unexpected first unset bit");
        }
    }
    ...
    if (nbits >= 3) {
        ...
        expect_zu_eq(
          bitmap_sfu(bitmap, binfo), nbits - 1, "Unexpected first unset bit");
    }
