...
edata_expect(edata_t *edata, size_t page_offset, size_t page_cnt) {
    ...
    expect_ptr_eq((void *)((uintptr_t)PAGESLAB_ADDR + (page_offset << LG_PAGE)),
...
stats_expect_empty(psset_bin_stats_t *stats) {
    assert_zu_eq(
      0, stats->npageslabs, "Supposedly empty bin had positive npageslabs");
...
stats_expect(psset_t *psset, size_t nactive) {
    if (nactive == HUGEPAGE_PAGES) {
        expect_zu_eq(
          1, psset->stats.full_slabs[0].npageslabs, "Expected a full slab");
    ...
    for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
        if (i == nonempty_pind) {
            assert_zu_eq(1, psset->stats.nonfull_slabs[i][0].npageslabs,
              "Should have found a slab");
            expect_zu_eq(nactive, psset->stats.nonfull_slabs[i][0].nactive,
...
TEST_BEGIN(test_insert_remove) {
    ...
    psset_update_begin(&psset, &pageslab);
    err =
      test_psset_alloc_reuse(&psset, &worse_alloc[HUGEPAGE_PAGES - 1], PAGE);
...
TEST_BEGIN(test_purge_prefers_nonhuge) {
    ...
    for (size_t i = 0; i < NHP; i++) {
        ...
        hpdata_init(
          &hpdata_nonhuge[i], (void *)((10 + NHP + i) * HUGEPAGE), 456 + i);
    ...
    for (int i = 0; i < NHP; i++) {
        ...
        assert_true(
          nonhuge_begin <= (uintptr_t)hpdata && (uintptr_t)hpdata < nonhuge_end,
          "");
        psset_update_begin(&psset, hpdata);
    ...
    for (int i = 0; i < NHP; i++) {
        hpdata = psset_pick_purge(&psset);
        expect_true(
          huge_begin <= (uintptr_t)hpdata && (uintptr_t)hpdata < huge_end, "");
        psset_update_begin(&psset, hpdata);
...
main(void) {
    return test_no_reentrancy(test_empty, test_fill, test_reuse, test_evict,
      test_multi_pageslab, test_stats, test_oldest_fit, test_insert_remove,
