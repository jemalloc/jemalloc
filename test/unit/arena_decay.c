...
TEST_BEGIN(test_decay_ticks) {
    ...
    expect_d_eq(mallctl("thread.arena", (void *)&old_arena_ind, &sz_arena_ind,
     ...
     */
    {
        ...
        for (i = 0; i < sizeof(allocx_sizes) / sizeof(size_t); i++) {
            ...
            expect_u32_ne(tick1, tick0,
              "Expected ticker to tick during mallocx() (sz=%zu)", sz);
            /* rallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            ...
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0,
              "Expected ticker to tick during rallocx() (sz=%zu)", sz);
            /* xallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            xallocx(p, sz, 0, MALLOCX_TCACHE_NONE);
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0,
              "Expected ticker to tick during xallocx() (sz=%zu)", sz);
            /* dallocx(). */
            tick0 = ticker_geom_read(decay_ticker);
            dallocx(p, MALLOCX_TCACHE_NONE);
            tick1 = ticker_geom_read(decay_ticker);
            expect_u32_ne(tick1, tick0,
              "Expected ticker to tick during dallocx() (sz=%zu)", sz);
            /* sdallocx(). */
            p = mallocx(sz, MALLOCX_TCACHE_NONE);
            expect_ptr_not_null(p, "Unexpected mallocx() failure");
            tick0 = ticker_geom_read(decay_ticker);
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("tcache.create", (void *)&tcache_ind, &sz, NULL, 0), 0,
    ...
    for (i = 0; i < sizeof(tcache_sizes) / sizeof(size_t); i++) {
        ...
        tick0 = ticker_geom_read(decay_ticker);
        expect_d_eq(mallctl("tcache.flush", NULL, NULL, (void *)&tcache_ind,
                      sizeof(unsigned)),
          0, "Unexpected mallctl failure");
        tick1 = ticker_geom_read(decay_ticker);
...
#define NPS 2048
    ...
    expect_d_eq(mallctl("arenas.lextent.0.size", (void *)&large, &sz, NULL, 0),
      0, "Unexpected mallctl failure");
    ...
    for (unsigned i = 0; i < NPS; i += 2) {
        ...
        do_decay(arena_ind);
        expect_u_gt(
          nupdates_mock, nupdates0, "Expected nstime_update() to be called");
    }

    decay_ticker_helper(
      arena_ind, flags, true, ddt, dirty_npurge0, muzzy_npurge0, true);
    decay_ticker_helper(
      arena_ind, flags, false, ddt + mdt, dirty_npurge0, muzzy_npurge0, false);
