...
#define NLARGE 32
    ...
    *ptrs = (void **)malloc(*nptrs * sizeof(void *));
    ...
    for (i = 0; i < nsmall; i++) {
        ...
        expect_ptr_not_null(
          (*ptrs)[i], "Unexpected mallocx(%zu, %#x) failure", sz, flags);
    }
    for (i = 0; i < nlarge; i++) {
        sz = get_large_size(i);
...
do_arena_reset_post(void **ptrs, unsigned nptrs, unsigned arena_ind) {
    ...
    for (i = 0; i < nptrs; i++) {
        expect_zu_eq(
          vsalloc(tsdn, ptrs[i]), 0, "Allocation should no longer exist");
    }
    if (have_background_thread) {
...
arena_i_initialized(unsigned arena_ind, bool refresh) {
    ...
    expect_d_eq(mallctlbymib(mib, miblen, (void *)&initialized, &sz, NULL, 0),
...
TEST_BEGIN(test_arena_destroy_hooks_default) {
    ...
    expect_d_eq(
      mallctl("arena.0.extent_hooks", (void *)&a0_default_hooks, &sz, NULL, 0),
    ...
    sz = sizeof(unsigned);
    expect_d_eq(mallctl("arenas.create", (void *)&arena_ind, &sz, (void *)&hook,
                  sizeof(void *)),
      0, "Unexpected mallctl() failure");
    do_arena_destroy(arena_ind);
}
