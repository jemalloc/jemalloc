...
TEST_BEGIN(test_decay_init) {
    ...
    expect_false(
      decay_init(&decay, &curtime, decay_ms), "Failed to initialize decay");
    expect_zd_eq(
      decay_ms_read(&decay), decay_ms, "Decay_ms was initialized incorrectly");
...
TEST_BEGIN(test_decay_ms_valid) {
    ...
    expect_true(
      decay_ms_valid(-1), "Misclassified -1 (never decay) as invalid decay");
    expect_true(decay_ms_valid(8943), "Misclassified valid decay");
    if (SSIZE_MAX > NSTIME_SEC_MAX) {
        expect_false(decay_ms_valid((ssize_t)(NSTIME_SEC_MAX * KQU(1000) + 39)),
...
TEST_BEGIN(test_decay_npages_purge_in) {
    ...
    expect_u64_eq(decay_npages_purge_in(&decay, &time, new_pages), new_pages,
      "Not all pages are expected to decay in decay_ms");
...
TEST_BEGIN(test_decay) {
    ...
    for (uint64_t i = nepoch_init; i * epoch_ns < decay_ns; ++i) {
        ...
        if (epoch_advanced) {
            ...
            expect_zu_lt(
              npages_limit_new, npages_limit, "napges_limit failed to decay");
...
TEST_BEGIN(test_decay_ns_until_purge) {
    ...
    uint64_t ns_until_purge_all =
      decay_ns_until_purge(&decay, dirty_pages, dirty_pages);
    ...
    uint64_t ns_until_purge_half =
      decay_ns_until_purge(&decay, dirty_pages, npages_threshold);
