...
static void
increment_push_failure(int batch) {
	assert_d_lt(batch, 3, "Only 3 batches per bin shard");
	if (batch == BATCHER_NO_IDX) {
...
static void
increment_pop_attempt(bool elems_to_pop) {
	atomic_fetch_add_zu(&pop_attempt_results[elems_to_pop], 1,
	    ATOMIC_RELAXED);
}
 ...
 */
static void
stress_run(void (*main_thread_fn)()) {
...
TEST_BEGIN(test_arena_reset) {
	...
	stress_run(&test_arena_reset_main_fn);
