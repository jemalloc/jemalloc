...
token_error(token_t *token) {
  ...
  switch (token->token_type) {
    ...
    case TOKEN_TYPE_ERROR:
      malloc_printf(
          "%zu:%zu: Unexpected character in token: ", token->line, token->col);
      break;
    default:
...
TEST_BEGIN(test_json_parser) {
  ...
  for (i = 0; i < sizeof(invalid_inputs) / sizeof(const char *); i++) {
    ...
    assert_false(
        parser_append(&parser, input), "Unexpected input appending failure");
    assert_true(
        parser_parse(&parser), "Unexpected parse success for input: %s", input);
    parser_fini(&parser);
  }
  ...
  for (i = 0; i < sizeof(valid_inputs) / sizeof(const char *); i++) {
    ...
    assert_false(
        parser_append(&parser, input), "Unexpected input appending failure");
    assert_false(
        parser_parse(&parser), "Unexpected parse error for input: %s", input);
    parser_fini(&parser);
  }
}
TEST_END
...
TEST_BEGIN(test_stats_print_json) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < sizeof(opts) / sizeof(const char *); j++) {
      ...
      assert_false(parser_parse(&parser), "Unexpected parse error, opts=\"%s\"",
          opts[j]);
      parser_fini(&parser);
    }
  }
