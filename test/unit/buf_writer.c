...
TEST_BEGIN(test_buf_write_dynamic) {
    buf_writer_t buf_writer;
    tsdn_t *tsdn = tsdn_fetch();
    assert_false(buf_writer_init(
                   tsdn, &buf_writer, test_write_cb, &arg, NULL, TEST_BUF_SIZE),
...
TEST_BEGIN(test_buf_write_oom) {
    buf_writer_t buf_writer;
    tsdn_t *tsdn = tsdn_fetch();
    assert_true(buf_writer_init(tsdn, &buf_writer, test_write_cb, &arg, NULL,
                  SC_LARGE_MAXCLASS + 1),
    ...
    for (unit = UNIT_MAX; unit >= 0; unit -= UNIT_MAX / 4) {
        ...
        for (n_unit = 1; n_unit <= 3; ++n_unit) {
            ...
            for (i = 1; i <= n_unit; ++i) {
                ...
                buf_writer_cb(&buf_writer, s);
                assert_u64_eq(
                  arg_store, arg, "Call back argument didn't get through");
