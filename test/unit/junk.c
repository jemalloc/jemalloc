...
arena_dalloc_junk_small_intercept(void *ptr, const bin_info_t *bin_info) {
  ...
  for (i = 0; i < bin_info->reg_size; i++) {
    assert_u_eq(((uint8_t *)ptr)[i], JEMALLOC_FREE_JUNK,
        "Missing junk fill for byte %zu/%zu of deallocated region", i,
        bin_info->reg_size);
  }
...
test_junk(size_t sz_min, size_t sz_max) {
  ...
  for (sz = sallocx(s, 0); sz <= sz_max; sz_prev = sz, sz = sallocx(s, 0)) {
    if (sz_prev > 0) {
      assert_u_eq(s[0], 'a', "Previously allocated byte %zu/%zu is corrupted",
          ZU(0), sz_prev);
      assert_u_eq(s[sz_prev - 1], 'a',
          "Previously allocated byte %zu/%zu is corrupted", sz_prev - 1,
          sz_prev);
    }
    ...
    if (xallocx(s, sz + 1, 0, 0) == sz) {
      ...
      assert_zu_ge(
          sallocx(t, 0), sz + 1, "Unexpectedly small rallocx() result");
