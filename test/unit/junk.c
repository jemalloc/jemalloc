...
TEST_BEGIN(test_junk_alloc_free) {
	...
	size_t sizevals[] = {1, 8, 100, 1000, 100*1000, 10 * 1000 * 1000};
	size_t lg_alignvals[] = {0, 4, 10, 15, 16, 20, 24, LG_PAGE};
	 ...
	 * rely on the 64-bit tests.  This is a little bit white-box-y in the
	 * sense that this is only a good test strategy if we know that the junk
	 * pathways don't touch interact with the allocation selection
	 * mechanisms; but this is in fact the case.
	 */
	test_skip_if(LG_SIZEOF_PTR == 2);
