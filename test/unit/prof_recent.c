...
#define ASSERT_FORMATTED_STR(s, ...) do {				\
	...
	ASSERT_STR(buf);						\
		...
		ASSERT_STR("\"alignment\"");
		ASSERT_CHAR(':');
		ASSERT_FORMATTED_STR("%zu", record->alignment);
		ASSERT_CHAR(',');

		ASSERT_STR("\"usize\"");
		ASSERT_CHAR(':');
		ASSERT_FORMATTED_STR("%zu", record->alignment == 0
		    ? sz_s2u(record->size)
		    : sz_sa2u(record->size, record->alignment));
		ASSERT_CHAR(',');
