...
TEST_BEGIN(test_prof_backtrace_hook_replace) {
    ...
    expect_d_eq(
      mallctl("experimental.hooks.prof_backtrace", (void *)&default_bt_hook,
    ...
    expect_d_eq(
      mallctl("experimental.hooks.prof_backtrace", (void *)&current_hook,
        &current_hook_sz, (void *)&default_bt_hook, sizeof(default_bt_hook)),
...
read_write_prof_sample_free_hook(prof_sample_free_hook_t *to_read,
  bool do_write, prof_sample_free_hook_t to_write) {
    ...
    expect_d_eq(mallctl("experimental.hooks.prof_sample_free", (void *)to_read,
                  &hook_sz, do_write ? &to_write : NULL, hook_sz),
...
check_prof_sample_hooks(bool sample_hook_set, bool sample_free_hook_set) {
    expect_false(
      mock_prof_sample_hook_called, "Should not have called prof_sample hook");
    ...
    expect_zu_eq(free_sampled_ptr_sz, 0, "Unexpected free sampled ptr size");
    ...
    if (sample_hook_set) {
        ...
        expect_zu_eq(alloc_sz, sampled_ptr_sz, "Unexpected sampled usize");
    }
