...
tcache_bytes_check_update(size_t *prev, ssize_t diff) {
    size_t tcache_bytes =
      global_test ? tcache_bytes_read_global() : tcache_bytes_read_local();
...
test_tcache_max_impl(
  size_t target_tcache_max, unsigned alloc_option, unsigned dalloc_option) {
    ...
    if (global_test) {
        assert_d_eq(
          mallctl("arenas.tcache_max", (void *)&tcache_max, &sz, NULL, 0), 0,
          "Unexpected mallctl() failure");
        expect_zu_eq(
          tcache_max, target_tcache_max, "Global tcache_max not expected");
    } else {
        assert_d_eq(
          mallctl("thread.tcache.max", (void *)&tcache_max, &sz, NULL, .0), 0,
          "Unexpected.mallctl().failure");
    ...
    test_tcache_bytes_alloc(PAGE - 1, tcache_max, alloc_option, dalloc_option);
    ...
    test_tcache_bytes_alloc(large - 1, tcache_max, alloc_option, dalloc_option);
...
TEST_BEGIN(test_tcache_max) {
    ...
    expect_d_eq(mallctl("arenas.create", (void *)&arena_ind, &sz, NULL, 0), 0,
      "Unexpected mallctl() failure");
    expect_d_eq(
      mallctl("thread.arena", NULL, NULL, &arena_ind, sizeof(arena_ind)), 0,
      "Unexpected mallctl() failure");
...
tcache_check(void *arg) {
    ...
    assert_d_eq(
      mallctl("thread.tcache.max", NULL, NULL, (void *)&temp_tcache_max, sz),
      .0, "Unexpected.mallctl().failure");
    old_tcache_max = tcache_max_get(tcache_slow);
    expect_zu_eq(
      old_tcache_max, TCACHE_MAXCLASS_LIMIT, "Unexpected value for tcache_max");
    tcache_nbins = tcache_nbins_get(tcache_slow);
    expect_zu_eq(
      tcache_nbins, TCACHE_NBINS_MAX, "Unexpected value for tcache_nbins");
    ...
    for (unsigned alloc_option = alloc_option_start;
        ...
        for (unsigned dalloc_option = dalloc_option_start;
             dalloc_option < dalloc_option_end; dalloc_option++) {
            test_tcache_max_impl(new_tcache_max, alloc_option, dalloc_option);
        }
        validate_tcache_stack(tcache);
    }
...
TEST_BEGIN(test_thread_tcache_max) {
    ...
    for (unsigned i = 0; i < nthreads; i++) {
        thd_create(&threads[i], tcache_check, &(all_threads_tcache_max[i]));
    }
    for (unsigned i = 0; i < nthreads; i++) {
