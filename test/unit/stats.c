...
TEST_BEGIN(test_stats_summary) {
	...
	expect_d_eq(mallctl("stats.metadata", (void *)&metadata, &sz, NULL, 0),
		expected, "Unexpected mallctl() result");
	expect_d_eq(mallctl("stats.metadata_edata", (void *)&metadata_edata, &sz, NULL, 0),
		expected, "Unexpected mallctl() result");
	expect_d_eq(mallctl("stats.metadata_rtree", (void *)&metadata_rtree, &sz, NULL, 0),
		expected, "Unexpected mallctl() result");
	...
	if (config_stats) {
		...
		expect_zu_le(
			...
			metadata,
			"the sum of metadata_edata and metadata_rtree should be no larger than metadata");
	}
}
TEST_END
