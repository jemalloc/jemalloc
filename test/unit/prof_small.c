...
assert_small_allocation_sampled(void *ptr, size_t size) {
    ...
    expect_ptr_eq(
      edata_base_get(edata), ptr, "Sampled allocations should be page-aligned");
    expect_zu_eq(
      edata_usize_get(edata), size, "Edata usize did not match requested size");
    ...
    expect_ptr_not_null(
      prof_tctx->tdata, "Edata had null prof_tdata despite being sampled");
}
