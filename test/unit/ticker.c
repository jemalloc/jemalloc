...
#define NTICKS 3
    ...
    for (i = 0; i < NREPS; i++) {
        ...
        expect_true(
          ticker_tick(&ticker, false), "Expected ticker fire (i=%d)", i);
...
#define NTICKS 3
    ...
    expect_true(ticker_ticks(&ticker, NTICKS, false), "Expected ticker fire");
    expect_u_eq(ticker_read(&ticker), NTICKS, "Unexpected ticker value");
...
#define NTICKS 3
    ...
    expect_true(ticker_ticks(&tb, NTICKS + 1, false), "Expected ticker fire");
    expect_u_eq(ticker_read(&tb), NTICKS, "Unexpected ticker value");
...
TEST_BEGIN(test_ticker_delay) {
    ...
    for (unsigned i = 0; i < niters; i++) {
        ...
        expect_false(
          ticker_geom_tick(&t2, &prng_state, delay), "Unexpected ticker fire");
        expect_d_eq(ticker_read(&t1), 0, "Unexpected ticker value");
        expect_d_eq(ticker_geom_read(&t2), 0, "Unexpected ticker value");
    }
