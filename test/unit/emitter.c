...
emit_dict(emitter_t *emitter) {
  ...
  emitter_kv_note(emitter, "ghi", "GHI", emitter_type_int, &i_123, "note_key1",
      emitter_type_string, &str);
  emitter_kv_note(emitter, "jkl", "JKL", emitter_type_string, &str, "note_key2",
      emitter_type_bool, &b_false);
...
main(void) {
  return test_no_reentrancy(test_dict, test_table_printf, test_nested_dict,
      test_types, test_modal, test_json_arr, test_table_row);
}
