...
do_fill_test(cache_bin_t *bin, void **ptrs, cache_bin_sz_t ncached_max,
  cache_bin_sz_t nfill_attempt, cache_bin_sz_t nfill_succeed) {
    ...
    for (cache_bin_sz_t i = 0; i < nfill_succeed; i++) {
        ...
        expect_true(cache_bin_low_water_get(bin) == nfill_succeed - i - 1, "");
    }
...
static void
do_flush_test(
  cache_bin_t *bin, void **ptrs, cache_bin_sz_t nfill, cache_bin_sz_t nflush) {
    bool success;
    assert_true(cache_bin_ncached_get_local(bin) == 0, "");
...
do_batch_alloc_test(
  cache_bin_t *bin, void **ptrs, cache_bin_sz_t nfill, size_t batch) {
    ...
    expect_true(cache_bin_low_water_get(bin) == nfill - (cache_bin_sz_t)n, "");
    while (cache_bin_ncached_get_local(bin) > 0) {
        bool success;
        cache_bin_alloc(bin, &success);
    }
...
TEST_BEGIN(test_cache_bin) {
    ...
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(cache_bin_low_water_get(&bin) == ncached_max - i, "");
        expect_true(cache_bin_ncached_get_local(&bin) == ncached_max - i, "");
        /*
        ...
        expect_true(cache_bin_low_water_get(&bin) == ncached_max - i - 1, "");
        expect_true(
...
static void
do_flush_stashed_test(
  cache_bin_t *bin, void **ptrs, cache_bin_sz_t nfill, cache_bin_sz_t nstash) {
    ...
    expect_true(cache_bin_ncached_get_local(bin) == 0, "Wrong cached count");
    expect_true(
      cache_bin_nstashed_get_local(bin) == nstash, "Wrong stashed count");
    ...
    expect_true(cache_bin_ncached_get_local(bin) == 0, "Wrong cached count");
    expect_true(cache_bin_nstashed_get_local(bin) == 0, "Wrong stashed count");
...
TEST_BEGIN(test_cache_bin_stash) {
    ...
    bool ret;
    for (cache_bin_sz_t i = 0; i < ncached_max; i++) {
        expect_true(cache_bin_ncached_get_local(&bin) == (i / 2 + i % 2),
          "Wrong ncached value");
        expect_true(
          cache_bin_nstashed_get_local(&bin) == i / 2, "Wrong nstashed value");
        if (i % 2 == 0) {
            cache_bin_dalloc_easy(&bin, &ptrs[i]);
        } else {
