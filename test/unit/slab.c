...
TEST_BEGIN(test_arena_slab_regind) {
    ...
    for (binind = 0; binind < SC_NBINS; binind++) {
        ...
        edata_init(&slab, INVALID_ARENA_IND,
          ...
          bin_info->slab_size, true, binind, 0, extent_state_active, false,
          true, EXTENT_PAI_PAC, EXTENT_NOT_HEAD);
        expect_ptr_not_null(
        ...
        for (regind = 0; regind < bin_info->nregs; regind++) {
            ...
            expect_zu_eq(arena_slab_regind(&dalloc_info, binind, &slab, reg),
              regind, "Incorrect region index computed for size %zu",
              bin_info->reg_size);
        }
